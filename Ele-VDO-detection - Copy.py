        ...
        for i in idxs.flatten():
            ...
            if f"{labels[class_ids[i]]}" == "elephant":
               # print("elephant")
                ...
                if len(collection_score) <= 5:
                    collection_score.append(confidences[i])

                    fileName = 'elephant_images{}.jpg'.format(i)
                    status = cv2.imwrite(fileName, fream)

                    if status == True:
                        collection_filePath.append(fileName)

                    print("[INFO] Saved a File ", status)
                    print(collection_score)
                elif len(collection_score) == 5:
                    maxFile = collection_filePath[collection_score.index(
                        max(collection_score))]
                    print(maxFile)
